...
onMounted(async () => {
    const { data } = await api.get(`${base}/${offerId}`);
        ...
        <div v-else class="grid grid-cols-1 gap-6 lg:grid-cols-3">
            ...
            <div class="space-y-6 lg:sticky lg:top-20 lg:self-start">
                ...
                <FullWidthBox title="Offer summary" :collapsible="false">
                    <table class="w-full border-collapse rounded border border-gray-300 text-sm">
                        <tbody>
                            ...
                            <tr v-if="offer.valid_to">
                                <td class="border border-gray-300 px-2 py-2 text-gray-500">Valid until</td>
                            ...
                            <tr v-if="offer.created_by">
                                <td class="border border-gray-300 px-2 py-2 text-gray-500">Prepared by</td>
                                <td class="border border-gray-300 px-2 py-2 text-right font-medium">
                                    {{ offer.created_by }}
                                    <span class="block text-xs font-normal text-gray-400">{{ offer.created_at }}</span>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                ...
                <div class="flex justify-end gap-2">
                    <RouterLink v-if="auth.can('tasksOffers.createOffer')"
                                :to="routeUrl('taskRequestOffers.edit', [taskId, offerRequestId, offerId])"
                                class="inline-flex items-center rounded border border-gray-300 bg-white px-4 py-1.5 text-sm hover:bg-gray-50">
                        Edit
                    </RouterLink>
                    <RouterLink :to="routeUrl('tasks.show', taskId)" class="inline-flex items-center rounded border border-gray-300 bg-white px-4 py-1.5 text-sm hover:bg-gray-50">Back to task</RouterLink>
                </div>
            </div>

            <!-- Right column: options per destination -->
            <div class="space-y-6 lg:col-span-2">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div class="inline-flex items-center gap-1 rounded-lg border border-gray-200 bg-white p-1 shadow-lg">
                        <button type="button" class="rounded px-4 py-1.5 text-sm font-medium"
                ...
                <section v-for="destination in destinations" :key="destination.id">
                    <template v-if="optionsFor(destination.id).length">
                        ...
                        <div v-if="detailed" class="space-y-4">
                            <div v-for="option in optionsFor(destination.id)" :key="option.id"
                                ...
                                <div class="mb-3 flex items-start justify-between gap-3">
                                    ...
                                    <button type="button" class="rounded px-3 py-1 text-xs font-medium"
                                            :class="isSelected(destination.id, option.id) ? 'bg-red-600 text-white' : 'border border-gray-300 text-gray-600 hover:bg-gray-50'"
                        ...
                        <div v-else class="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <button v-for="option in optionsFor(destination.id)" :key="option.id" type="button"
                                    class="rounded-lg border-2 bg-white p-5 text-left shadow-lg transition-colors"
                                    :class="isSelected(destination.id, option.id) ? 'border-red-600 ring-1 ring-red-600' : 'border-gray-200 hover:border-gray-300'"
                                    @click="select(destination.id, option.id)">
                                <div class="flex items-start justify-between gap-2">
                                    <h3 class="font-semibold">{{ option.name }}</h3>
                                    <span v-if="isSelected(destination.id, option.id)" class="rounded bg-red-600 px-2 py-0.5 text-xs font-medium text-white">Selected</span>
                                </div>

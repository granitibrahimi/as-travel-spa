...
const { filters, response: apiResponse, loading, apply, clear, goToPage, reload } = useListFilters(
    { q: '', open: false },
    async (params, { signal }) => castPaginated((await api.get('/suppliers/refunds', { params, signal })).data),
);
    ...
    <AppLayout title="Supplier Reimbursements" fluid>
        <FullWidthBox title="Supplier Reimbursements" :collapsible="false">
            <form class="mb-4 flex flex-wrap items-end gap-2" @submit.prevent="apply">
                <input v-model="filters.q" type="text" placeholder="Gen ID, transaction #…" class="w-full rounded border border-gray-300 px-3 py-1.5 text-sm focus:border-red-500 focus:ring-1 focus:ring-red-500 sm:w-72">
        ...
        <ConfirmDialog
            ...
            :message="toDelete ? `Reimbursement ${toDelete.gen_id} will be permanently deleted.` : ''"

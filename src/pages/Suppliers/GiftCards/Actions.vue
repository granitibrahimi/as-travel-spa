...
// Reusable supplier-gift-card actions side overlay (mirrors
// Suppliers/Payments/Actions.vue). Actions are defined here, not served by the
...
//     (modules/Suppliers routes). There is no PDF/print endpoint and no
//     `qb_link` on the response, so the "Receipt" and "QB" actions from
//     Payments have no gift-card equivalent and are omitted.
//   - Reconcile reuses the supplier-level Reconcile page, gated on
//     `suppliers.reconcile` — same as Payments/Actions.vue.
//   - Journal slug is `supplier-gift-card`
//     (AccountTransactionType::SUPPLIER_GIFT_CARD->slug()).
...
const groups = computed(() => {
    ...
    const supplierId = giftCard.supplier?.id ?? null;
    const result = [];

    const record = [
        ...(props.showViewAction
            ? [{ label: 'View', to: routeUrl('supplierGiftCards.show', giftCard.id), can: 'supplierGiftCards.show' }]
            : []),
        ...(supplierId
            ? [{ label: 'Reconcile', to: routeUrl('suppliers.reconcile', supplierId), can: 'suppliers.reconcile' }]
            : []),
        { label: 'Edit', to: routeUrl('supplierGiftCards.edit', giftCard.id), can: 'supplierGiftCards.edit' },
        ...(props.showAddDocument
            ? [{ label: 'Add document', action: () => emit('addDocument'), can: 'supplierGiftCards.edit' }]
            : []),
        { label: 'Delete', danger: true, action: () => (toDelete.value = giftCard), can: 'supplierGiftCards.delete' },
    ...
    const links = [
        { label: 'Journal', to: `/finance/account-transactions/journal/supplier-gift-card/${giftCard.id}`, can: 'accountTransactions.journal' },
...
async function confirmDelete() {
    ...
    try {
        ...
        await api.delete(`/suppliers/gift-cards/${removed.id}`);
        toDelete.value = null;

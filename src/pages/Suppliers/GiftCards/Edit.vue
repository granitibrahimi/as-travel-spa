...
// on_date is not editable but the backend update still validates it as
// required (date_format:d.m.Y), so it is preserved and sent back verbatim.
const form = reactive({
    amount: null,
    notes: '',
});
...
onMounted(async () => {
    ...
    supplier.value = giftCard.supplier ?? null;
    loaded.value = true;
});
    ...
    <AppLayout title="Edit gift card" fluid>
        ...
        <form v-else class="space-y-6" @submit.prevent="submit">
            ...
            <footer
                ...
                <RouterLink :to="routeUrl('supplierGiftCards.show', giftCardId)"

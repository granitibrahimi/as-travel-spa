...
// Same list as the Open Payments page — GET /suppliers/payments?open=1 —
...
const { response: apiResponse, loading, goToPage, reload } = useListFilters(
    {},
    async (params, { signal }) => castPaginated((await api.get('/suppliers/payments', { params: { ...params, open: 1, per_page: 500 }, signal })).data),
);
...
// matches id, gen id, supplier id/name, date and the amounts — typed either
...
async function submit() {
    ...
    try {
        await api.post('/suppliers/payments/reconcile-unused', {
...
<template>
    <AppLayout title="Reconcile Unused Payments" fluid>
        <DashboardWidget title="Reconcile Unused Supplier Payments">
            <template #actions>
            ...
            <div class="mb-4 max-w-sm">
                <InputText v-model="search" placeholder="Search by ID, supplier, amount, date…" />
            </div>
                ...
                <table class="w-full border-collapse border border-gray-300 text-sm">
                    ...
                    <tbody>
                        ...
                        <tr v-for="payment in rows" :key="payment.id" class="hover:bg-gray-50" :class="{ 'bg-red-50/40': isSelected(payment) }">
                            ...
                            <td class="border border-gray-300 px-2 py-2">
                                <div v-if="payment.supplier" class="flex items-center gap-2">
                                    <button
                                        type="button"

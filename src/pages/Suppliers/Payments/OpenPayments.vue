...
const { response: apiResponse, loading, goToPage } = useListFilters(
    {},
    async (params, { signal }) => castPaginated((await api.get('/suppliers/payments', { params: { ...params, open: 1 }, signal })).data),
);
...
<template>
    <AppLayout title="Open Supplier Payments" fluid>
        <DashboardWidget title="List of Open Payments">
            <template #actions>
                <Button
                    v-if="auth.can('suppliers.reconcile')"
                    size="sm"
                    :href="routeUrl('supplierPayments.reconcileUnused')"
                    @click.prevent="router.push(routeUrl('supplierPayments.reconcileUnused'))"
                >

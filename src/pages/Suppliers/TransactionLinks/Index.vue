...
import { supplierTransactionPathForLabel } from '../../../helpers/supplierTransactions.js';
...
const { filters, response: apiResponse, loading, apply, clear, goToPage, reload } = useListFilters(
    { q: '' },
    async (params, { signal }) => castPaginated((await api.get('/suppliers/transaction-links', { params, signal })).data),
);
...
function parentPath(link) {
    return link.is_reconciliation
        ? routeUrl('supplierReconciliations.show', link.reconciliation_id)
        : routeUrl('supplierPayments.show', link.payment_id);
}
...
const rowActions = (link) => [
    ...(auth.can('supplierTransactionsLinks.unlink')
...
<template>
    <AppLayout title="Supplier Transactions" fluid>
        <FullWidthBox title="Supplier Transactions" :collapsible="false">
            <form class="mb-4 flex flex-wrap items-end gap-2" @submit.prevent="apply">
                <input v-model="filters.q" type="text" placeholder="Supplier name or link ID…" class="w-full rounded border border-gray-300 px-3 py-1.5 text-sm focus:border-red-500 focus:ring-1 focus:ring-red-500 sm:w-72">
                    ...
                    <tbody>
                        ...
                        <tr v-for="link in (loading ? [] : apiResponse?.data ?? [])" :key="link.id" class="hover:bg-gray-50">
                            ...
                            <td class="border border-gray-300 px-2 py-2">
                                <RouterLink
                                    v-if="supplierTransactionPathForLabel(link.type, link.transaction_id)"
                                    :to="supplierTransactionPathForLabel(link.type, link.transaction_id)"
                                    class="text-red-600 hover:underline"
                                >{{ link.reference }}</RouterLink>
                                <span v-else>{{ link.reference }}</span>
                            </td>
                            <td class="border border-gray-300 px-2 py-2">
                                <RouterLink v-if="link.supplier" :to="routeUrl('suppliers.show', link.supplier.id)" class="text-red-600 hover:underline">{{ link.supplier.id }} # {{ link.supplier.name }}</RouterLink>
                                <span v-else>—</span>
                            </td>
                            <td class="border border-gray-300 px-2 py-2">

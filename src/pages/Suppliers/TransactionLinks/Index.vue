...
async function fetchLinks(page = 1) {
    ...
    try {
        const { data } = await api.get('/suppliers/transaction-links', { params: { q: search.value || undefined, page } });
    ...
    <AppLayout title="Supplier Transactions" fluid>
        <FullWidthBox title="Supplier Transactions" :collapsible="false">
            <form class="mb-4 flex flex-wrap items-end gap-2" @submit.prevent="fetchLinks()">
                <input v-model="search" type="text" placeholder="Supplier name or link ID…" class="w-full rounded border border-gray-300 px-3 py-1.5 text-sm focus:border-red-500 focus:ring-1 focus:ring-red-500 sm:w-72">

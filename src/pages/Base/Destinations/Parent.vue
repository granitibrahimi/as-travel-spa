...
import api, {getParentDestinationsAutosuggestEndpoint} from '../../../helpers/api.js';
...
// Parent-destination lookup lives under /api/customers (sibling of the /api/v1
// api client base), so addressed with an absolute URL off the origin.
...
async function submit() {
    ...
    if (! parentDestinationId.value) {
        error.value = 'Choose the parent destination.';

        return;
    }

    if (selected.value.size === 0) {
        error.value = 'Select at least one destination to reassign.';
    ...
    try {
        await api.post('/destinations/parent', {
            parent_destination_id: parentDestinationId.value,
            child: [...selected.value],
        });
        router.push(routeUrl('destinations.list'));
    } catch (e) {
        error.value = e.response?.data?.message ?? 'Reassignment failed.';
    } finally {
...
<template>
    <AppLayout title="Reassign parent destination">
        <FullWidthBox title="Reassign parent destination" :collapsible="false">
            ...
            <div class="mb-4 grid grid-cols-1 gap-4 md:grid-cols-2">
                <AsyncSelect
                    ...
                    label="Search parent destination…"
                ...
                <table class="w-full border-collapse border border-gray-300 text-sm">
                    ...
                    <tbody>
                        ...
                        <tr v-else-if="apiResponse.data.length === 0">
                            <td colspan="4" class="border border-gray-300 px-2 py-4 text-center text-gray-400">No destinations found.</td>
                        </tr>
            ...
            <template #footer>
                <div class="flex items-center gap-3">
                    ...
                    <Button variant="primary" :disabled="processing" @click="submit">
                        {{ processing ? 'Saving…' : 'Reassign selected' }}
                    </Button>
                </div>
            </template>
        </FullWidthBox>

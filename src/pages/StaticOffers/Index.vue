...
const rowActions = (offer) => [
    { label: 'View', href: `/offers/${offer.id}` },
    ...(auth.can('staticOffers.create') ? [{ label: 'Edit', href: `/offers/${offer.id}/edit` }] : []),
...
async function confirmDelete() {
    ...
    try {
        ...
        await fetchOffers(offers.value?.current_page ?? 1);
    } finally {
        ...
        <FullWidthBox title="Static offers" :collapsible="false">
            ...
            <div class="overflow-x-auto">
                <table class="w-full border-collapse border border-gray-300 text-sm">
                    ...
                    <tbody>
                        <tr v-if="loading || ! offers">
                            <td colspan="10" class="border border-gray-300 px-2 py-2"><Loader /></td>
                        </tr>
                        <tr v-else-if="offers.data.length === 0">
                            <td colspan="10" class="border border-gray-300 px-2 py-4 text-center text-gray-400">
                                No offers found.
                                <RouterLink v-if="auth.can('staticOffers.create')" to="/offers/create" class="text-red-600 hover:underline">Add one</RouterLink>
                            </td>
                        </tr>
                        <tr v-for="offer in (loading ? [] : offers?.data ?? [])" :key="offer.id" class="hover:bg-gray-50">
            ...
            <ApiPagination v-if="offers" :paginator="offers" class="mt-4" @page="fetchOffers" />

            <template #footer>
                <RouterLink v-if="auth.can('staticOffers.create')" to="/offers/create" class="inline-block rounded bg-red-600 px-3 py-1 text-sm text-white hover:bg-red-700">
                    + New static offer
                </RouterLink>
            </template>
        </FullWidthBox>

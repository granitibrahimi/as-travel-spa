...
// Generate an online payment link for a pro-invoice (mirrors
// Customers/Invoices/PaymentLinkModal.vue — same flow, pro-invoice endpoint).
//
// TODO: endpoint is a best-effort guess by symmetry with
// /customers/invoices/{id}/payment-link — confirm once available.
...
watch(() => props.show, (open) => {
    if (open) {
        ...
        link.value = '';
        copied.value = false;
        amount.value = props.proInvoice?.amount ?? '';
    }
});

async function generate() {
    if (generating.value || !props.proInvoice) {
        return;
    }
    ...
    try {
        const { data } = await api.post(`/customers/pro-invoices/${props.proInvoice.id}/payment-link`, {
        ...
        <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
                <h3 class="text-lg font-semibold text-gray-900">
                    Payment link<span v-if="proInvoice"> · {{ proInvoice.gen_id }}</span>
                </h3>

                <div v-if="!link" class="mt-4 space-y-3">
                    <InputText v-model="amount" label="Amount" type="number" placeholder="Amount" />

...
// Edit a customer credit note's orders and their persons. Header (customer/date)
// is read-only here. Mirrors the platform UpdateCustomerCreditNoteAction payload:
// orders[] with nested persons[], dates in d.m.Y — the same shape as customer
// invoices. Supplier/destination use the autosuggest endpoints; products come
// from the shared formOptions store.
...
// Sold values only apply when nothing has been used yet (matches the backend).
const soldLocked = computed(() => (creditNote.value?.paid_amount ?? 0) != 0);

// Live credit note total from the persons' sold values.
...
<template>
    <AppLayout :title="creditNote ? `Edit credit note ${creditNote.gen_id}` : 'Edit credit note'" fluid>
        <Loader v-if="loading" />

        <template v-else>
            <FullWidthBox :title="`Edit credit note ${creditNote.gen_id}`" :collapsible="false" class="mb-6">
                <dl class="grid grid-cols-1 gap-x-8 gap-y-2 text-sm sm:grid-cols-3">
                    <div class="flex gap-2"><dt class="w-24 shrink-0 font-medium text-gray-500">Customer</dt><dd>{{ creditNote.customer?.name ?? '—' }}</dd></div>
                    <div class="flex gap-2"><dt class="w-24 shrink-0 font-medium text-gray-500">Date</dt><dd>{{ creditNote.on_date }}</dd></div>
                ...
                <p v-if="soldLocked" class="mt-3 rounded bg-amber-50 px-3 py-2 text-xs text-amber-700">
                    This credit note has been used — sold values are locked and won't be changed.
                </p>
                        ...
                        <table class="w-full min-w-[560px] border-collapse text-sm">
                            ...
                            <tbody>
                                <tr v-for="(person, pi) in order.persons" :key="pi" class="align-top">
                                    ...
                                    <td class="px-2 py-2"><InputNumber v-model="person.sold_value" :disabled="soldLocked" /></td>

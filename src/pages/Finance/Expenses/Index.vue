...
async function fetchExpenses(page = 1) {
    ...
    try {
        ...
        expenses.value = { data: data.data, ...data.pagination };
    } catch (error) {
...
async function confirmDelete() {
    ...
    try {
        ...
        toDelete.value = null;
        await fetchExpenses(expenses.value?.current_page ?? 1);
    } finally {
...
const rowActions = (expense) => [
    ...(auth.can('expenses.show') ? [{ label: 'View', href: `/expenses/${expense.id}` }] : []),
    ...(auth.can('expenses.edit') ? [{ label: 'Edit', href: `/expenses/${expense.id}/edit` }] : []),
...
<template>
    <AppLayout title="Expenses" fluid>
        <FullWidthBox title="Expenses" :collapsible="false">
            ...
            <div class="overflow-x-auto">
                <table class="w-full border-collapse border border-gray-300 text-sm">
                    ...
                    <tbody>
                        ...
                        <tr v-else-if="expenses.data.length === 0">
                            <td colspan="6" class="border border-gray-300 px-2 py-4 text-center text-gray-400">No expenses found.</td>
                        </tr>
                        <tr v-for="expense in (loading ? [] : expenses?.data ?? [])" :key="expense.id" class="hover:bg-gray-50">
            ...
            <ApiPagination v-if="expenses" :paginator="expenses" class="mt-4" @page="fetchExpenses" />

            <template #footer>
                <RouterLink v-if="auth.can('expenses.create')" to="/expenses/create" class="inline-block rounded bg-red-600 px-3 py-1 text-sm text-white hover:bg-red-700">
                    + Expense
                </RouterLink>
            </template>
        </FullWidthBox>

...
const props = defineProps({
    ...
    // Local options as [{ value, label }] — filtered client-side, no backend calls.
...
const initialLabel = props.options.find((option) => option.value === props.modelValue)?.label ?? '';
const search = ref(initialLabel);
const selectedLabel = ref(initialLabel);
...
const results = computed(() => {
    ...
    // Show the full list when nothing is typed or the current selection is shown.
    if (!term || search.value === selectedLabel.value) {
        ...
        <div class="relative">
            <div ref="wrapperRef" class="relative">
                <input
                    ...
                    role="combobox"
                    :aria-expanded="open"
                    class="w-full rounded border py-1.5 pl-3 pr-9 text-base font-normal leading-normal focus:border-red-500 focus:ring-1 focus:ring-red-500 disabled:bg-gray-100"

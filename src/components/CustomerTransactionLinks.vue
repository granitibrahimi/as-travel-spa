...
const props = defineProps({
    ...
    // Opt-in "Customer" column, shown where rows span multiple customers (the
    // Journal show page). Each row must then carry `party: { id, name }`.
    // Redundant on a single customer's own show page, so off by default.
...
// Columns left of "Amount" (ID, Type, [Customer], Reference, Date) — drives the
        ...
        <table class="w-full border-collapse border border-gray-300 text-sm">
            <thead>
                <tr class="border-b text-left text-gray-500">
                    ...
                    <th v-if="showParty" class="border border-gray-300 px-2 py-2">Customer</th>
            ...
            <tbody>
                <tr v-for="(link, i) in links" :key="link.id ?? i" class="border-b last:border-0 hover:bg-gray-50">
                    ...
                    <td v-if="showParty" class="border border-gray-300 px-2 py-2">
                        <RouterLink
                            v-if="link.party?.id"
                            :to="routeUrl('customers.show', link.party.id)"
